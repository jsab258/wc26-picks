...
| **waiting on Jafar** | one more re-pick run | **HE RAN IT, 21 Aug: 59 copied, 3 substituted, 8 missing**, fifteen slots replaced, and all thirteen clips it removed fail the screen — so the screen drove every one of them. `walk` contains a walk now, `idle` and `talk` stand still. **What is left is one more run of the same batch file**, because reading the result found two things the first run could not have fixed. **A slot that found nothing kept serving the clip the screen had just refused** — the picker cleared a slot before copying and had no such step on the failure path, so eight slots were still loading a refused motion while `_picks.json` correctly said `found: null` and nothing reads `_picks.json`. Those are set aside now, which empties the slot rather than filling it wrongly; `HasState` sends the body back to the locomotion tree, so a person stands normally instead of playing somebody else's motion. **And six of the eight holes had alternatives the patterns never tried** — `--dryrun` reads 0 missing where it read 8, which is a claim about NAMES only, since the dryrun has no files and the screen decides on his machine. `smoke` and `thinking` have exactly one name each in all 2,846 and both travel: a harvest hole, not a pick hole. No purchase, no download, no token. |
...
| **Law as a tool** | 40 | 70 | **the verb exists as of 4 Aug.** `Core/Informing` weighs an accusation against what the street would tell a detective, on the same magistrate's bar Act III uses, and returns the mark that goes on the player for having informed. `Core/Claims` turns a typed alibi into a `Fact`, so `ProcessClaim` and `PlayerClaims` run for the first time. **Redirecting an inquiry landed too (4 Aug).** `HomicideBook.PointAt` stores who the law is asking about instead of you; the relief comes off the NAMED half of the pressure only, never off the bodies, so a charge that sticks walks a manhunt back to an investigation and can never walk it to nothing — 1.00 to 0.73, then 0.80, 0.87, 0.93 and back to 1.00 over four days. `EvidenceHost.InquiryOf` was a second implementation of the same arithmetic and now delegates. **THE CHAIN RUNS AND THE SURFACE EXISTS — re-measured 22 Aug against the landed verdict, correcting this row's 4-Aug "no callers" claim which had gone stale in the other direction.** The homicide register fills (a staged killing lands, eight witnesses hold it), the inquiry rises to Manhunt, the paper names the publican, and a stuck charge redirects it (one redirect at stage Manhunt in the landed run). The player accuses FROM the intent router: an inform intent calls the same weighing the sim's denounces use, with the predicate fixed so what lands is what the street will back, never what the player asserts. Still missing: a discoverable surface — the verb lives only inside free conversation, and nothing in the world (callbox, station door) advertises it |
