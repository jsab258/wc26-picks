...
| 17.1b | **Bodies and faces for EVERYONE** | new 2026-08-01. **THE TOWN IS NO LONGER BOXES — wired 2026-08-04, first build pending.** `RealBody.TryAttach` had exactly ONE caller, `PlayerController`, so the player was a person and all sixty-seven walkers were articulated boxes; the named cast now gets skinned bodies and the anonymous crowd keeps mannequins by choice. It needed a prerequisite: `TryAttach` publishes statics that five clauses of the `bodies` gate read as THE PLAYER's, so a separate path saves and restores them — without it, attaching walkers would have silently made all five describe the last walker, and a corrupted gate reads exactly like a passing one. **Not claimed as done until `walkerBodies` comes back non-zero.** Gait bias, bad leg and idle phase are carried across from `Physique.For` — the same deterministic source `Mannequin` uses — so the two tiers cannot disagree about who walks how and the cast does not walk in unison. **HALF CLOSED 2026-08-04: eight real Mixamo bodies are in, `bodyChoices=10`, and the player renders as a human figure.** What is left is the SKIN, and it is a one-line import fault rather than an asset gap — see the `now` row. The paragraph below is kept because its lesson stands and its conclusion does not. **2026-08-03, from the frame: the player is not "skinned but undressed", it is Mixamo's free `X Bot` mannequin** — `CharacterPrefab.BodyModel` names it, and the 44 imported files are animation clips carrying no mesh of their own. So `bodyDressed=1` is true and useless: the coat went on, onto a placeholder. The whole town is coloured boxes and the one skinned body is a shop dummy. Buying a real character mesh is Jafar's call and nothing here substitutes for it | **open — the biggest immersion gap** |
