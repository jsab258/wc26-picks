...
namespace Ledger.BalanceLab
{
    ...
    static class Program
    {
        ...
        class Policy
        {
            ...
            public bool CollectDebts;  // work Mickey's book on day 2
            ...
            RunWeek(Policy policy, Random rng)
        {
            ...
            if (policy.Purses)
            {
                ...
                purses.Add(new Purse { OwnerId = "Sam", Name = "Sam", Weekly = 60, Ceiling = 95, Cash = 45, PatronId = "Donna" });
            ...
            RunOpenCampaign(OpenPlan plan, Random rng)
        {
            ...
            openPurses.Add(new Purse { OwnerId = "Sam", Name = "Sam", Weekly = 60, Ceiling = 95, Cash = 45, PatronId = "Donna" });
            openPurses.Add(new Purse { OwnerId = "Rocco", Name = "Rocco", Weekly = 140, Ceiling = 260, Cash = 180 });
            openPurses.Add(new Purse { OwnerId = "Donna", Name = "Donna", Weekly = 220, Ceiling = 520, Cash = 380 });
            ...
            books.EllisCaseAnswerable = mill.StrongestSurvivingPlayerLead() < LedgerState.CaseStandsAt;
            ...
            foreach (var plan in new[] { OpenPlan.Control, OpenPlan.Aggressive, OpenPlan.Cautious })
            {
                      ...
                      ("stonewalled", 0, 3, false), ("answered+deflect", 5, 0, true) })
                {
                    ...
                    for (int seed = 0; seed < runs; seed++)
                    {
                        ...
                        books.EllisCaseAnswerable = books.EllisCaseAnswerable || deflected;
        ...
        static void PlanActions(OpenPlan plan, EmpireBook e, GossipMill mill, Wallet wallet, GameTime now)
        {
            ...
            var viktor = mill.Get("Victor");
            var josip = mill.Get("Joey");
            ...
            if (plan == OpenPlan.Aggressive)
            {
                ...
                if (e.CrewOf("Joey") == null && josip != null && wallet.Total >= 100) e.RecruitByNeed(josip, "Joey", 100, wallet, now);
                var prot = e.RacketOf("protection");
                if (!prot.Established && e.CrewOf("Joey") != null) e.Establish(prot, e.CrewOf("Joey"), now);
            }
        ...
        static bool Active(string id, int hour)
        {
            switch (id)
            {
                ...
                case "Marla": return hour >= 8 && hour < 18;  // the stall
                case "Victor": return hour >= 9 && hour < 20;  // shop hours, then the teahouse
                case "Joey": return hour >= 18 || hour < 6;   // the docks' hours
