...
namespace Ledger.Game
{
    ...
    public class SimDirector : MonoBehaviour
    {
        ...
        /// The emission-slot probe LIVED HERE AND IS GONE (22 Aug, one
        /// build). Its own accepting case failed — built-in white read the
        /// same 0.09 as every other source, so by its own rule none of its
        /// numbers could be believed — and the SCENE settled the question
        /// it was built for: three landed builds showed that ANY texture
        /// bound into this player's emission slot kills the glow, built-in
        /// white included, while the unbound slot glowed for weeks. No
        /// emission map is bound anywhere now (WorldBuilder.AddWindow has
        /// the account), so the question is closed and a broken instrument
        /// does not get explained, it gets deleted.
