...
WANTS = [
    ...
    ("guard",        "A", [r"\bboxing idle\b", r"\bfight(ing)? idle\b",
                           r"\bstanding melee combat idle\b", r"\bboxing\b"]),
    ("strike",       "A", [r"\bcross punch\b", r"\bjab\b", r"\bpunch(ing)?\b",
                           r"\bhook punch\b"]),
    ("shove",        "A", [r"\bpush(ing)?\b", r"\bshove\b"]),
    ...
    ("stagger",      "A", [r"\bstanding react large\b", r"\bstumbl(e|ing)\b",
                           r"\bstagger\b", r"\bknocked back\b"]),
    ("knockdown",    "A", [r"\bknocked out\b", r"\bfalling back death\b",
                           r"\bknock(ed)? down\b", r"\bdeath from front\b"]),
    ("get_up",       "A", [r"\bstand(ing)? up\b", r"\bgetting up\b",
                           r"\bget up\b", r"\brising\b"]),
    ("draw_reach",   "A", [r"\bdraw(ing)? sword\b", r"\bsword draw\b",
                           r"\bunsheath", r"\bdraw\b"]),
    ("draw_holster", "A", [r"\bsheath(e|ing)? sword\b", r"\bsheath",
                           r"\bholster\b", r"\bput(ting)? away\b"]),
    ("idle",         "A", [r"\bbreathing idle\b", r"\bstanding idle\b",
                           r"^idle\b", r"\bidle\b"]),
    ("walk",         "A", [r"^walking\b", r"\bwalk(ing)? forward\b", r"\bwalk(ing)?\b"]),
    ("run",          "A", [r"^running\b", r"\brun(ning)? forward\b", r"\brun(ning)?\b"]),
    ...
    ("turn_right",   "B", [r"\bstanding turn right 90\b", r"\bright turn\b",
                           r"\bturn(ing)? right\b"]),
    ("look_around",  "B", [r"\blook(ing)? around\b", r"\bsearch(ing)?\b",
                           r"\blook(ing)? over shoulder\b"]),
    ("talk",         "B", [r"^talking\b", r"\btalk(ing)?\b", r"\bconversation\b"]),
    ("greet",        "B", [r"\bstanding greeting\b", r"\bwav(e|ing)\b",
                           r"\bgreeting\b"]),
    ("flinch",       "B", [r"\bterrified\b", r"\bscared\b", r"\bflinch\b",
                           r"\bstartl(e|ed)\b"]),
    ("hands_up",     "B", [r"\bsurrender\b", r"\bhands up\b", r"\barms up\b"]),
    ("back_away",    "B", [r"\bwalk(ing)? backward", r"\bback(ing)? up\b",
                           r"\bstep(ping)? back\b"]),
    ("walk_start",   "B", [r"\bwalk(ing)? start\b", r"\bstart walking\b"]),
    ("walk_stop",    "B", [r"\bwalk(ing)? stop\b", r"\bstop walking\b",
                           r"\bwalk(ing)? to stop\b"]),

    # ---- TIER C: life, and the end -------------------------------------
    ("sit",          "C", [r"\bsitting idle\b", r"^sitting\b", r"\bsit(ting)?\b"]),
    ("lean",         "C", [r"\blean(ing)? against wall\b", r"\blean(ing)?\b"]),
    ("drink",        "C", [r"\bdrink(ing)?\b"]),
    ("smoke",        "C", [r"\bsmok(e|ing)\b"]),
    ("work_counter", "C", [r"\bcounter\b", r"\btyping\b", r"\bwriting\b",
                           r"\bstanding idle\b"]),
    ("collapse",     "C", [r"\bdying\b", r"\bfalling back death\b", r"\bdeath\b"]),
    ("fall_stairs",  "C", [r"\bfall(ing)? down stairs\b", r"\bstair.*fall\b",
                           r"\bfalling down\b"]),
    ("lie_still",    "C", [r"\blying\b", r"\bdead\b", r"\bidle.*ground\b"]),
]
